...
class EZO_RTD:
    ...
    def set_scale(self, scale):
        ...
        if scale not in list(scales.keys()):
            ...
            return 1
        rsp,payload,n = self.command_rw(scales[scale], 0.5, 6)
        if scale is '?':
            if payload == b'?S,F':
                return 'fahrenheit'
            elif payload == b'?S,K':
                ...
                return 'ERROR: payload ={}'.format(payload)


    def take_reading(self):
        r,p,n = self.command_rw(CMD_R, 0.7, 8)
        return float(p)

    def device_info(self):
        r,p,n = self.command_rw(CMD_I, 0.5, 13)
        rsp, device, firmware = p.split(',')
        return device, firmware

    def command_rw(self, cmd, delay, n):
        self._write(cmd)
        time.sleep(delay)
        data = self._read(n)
        return data[0], data[1:n-1], data[n-1]    # repsonse, payload, null
